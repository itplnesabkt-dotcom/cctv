...
export class GoogleSheetsService {
  ...
  private static async fetchSheetDataRaw(sheetName: string): Promise<any[][]> {
    const url = `/api/sheets?sheetName=${encodeURIComponent(sheetName)}`;
  ...
  private static compileDashboardDataFromRaw(
    ...
    return {
      ...
      cctvUsage,
      summary,
      allUlps: standardUlps,
      allPoskos: standardUlps.map(u => `POSKO ${u}`),
      overSla,
      rating,
      anomali,
      vccData: vccRows,
  ...
  private static generateMockDashboardData(
      ...
      officers.forEach((officer) => {
        ...
        for (let i = 0; i < jobsCount; i++) {
          ...
          rawWoRows.push([
            ...
            rpt.toString(), // 8: RPT
            rct.toString(), // 9: RCT
            officer.name, // 10: Nama Petugas (match woIndices.name = 10 fallback)
            shift, // 11: Shift
      ...
      officers.forEach((officer) => {
        ...
        for (let i = 0; i < poCount; i++) {
          ...
          rawPoRows.push([
            ...
            ulp,
            officer.name,
            isCctvUsed ? "CCTV YA" : "TIDAK PAKAI CCTV",
            "PENDING"
          ]);
        }
      });
    ...
    return this.compileDashboardDataFromRaw(rawWoRows, rawPoRows, [["Timestamp", "NOMOR WO YANTEK DENGAN CCTV", "POSKO ULP", "USER REGU", "TANGGAL WO", "Anomali", "KETERANGAN ANOMALI"]], startDateStr, endDateStr, selectedUlp);
  }
}
